...
class TweetsClustering:
    ...
    def preprocessTweet(self):
        '''with open("Tweets.json.txt") as t:
            for line in t:
                tweet = json.loads(line)
                #self.tweets[tweet["id"]] = tweet
                tweettext = tweet["text"]
                #result = re.search('"text":(.*)"profile_image_url":', line)
                #result = result.group(1)
                words = tweettext.lower().strip().split()
                if words[0] == 'rt':
                    words = words[2:]
                #else:
                #    words[0] = words[0][1:]
                #print (words)
                self.tweets[tweet["id"]] = words
	...
	for tweet in f:
		...
		self.parsed_tweets[count] = parsed_tweet
    ...
    def getJaccard(self):
        for t1 in self.tweets:
			...
			print("si: ", t1, " s2: ", t2, "union: ", s1.union(s2))
